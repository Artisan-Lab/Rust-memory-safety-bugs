...
\multirow{2}{*}{ozone } | Advisory-DB | RUSTSEC-2020-0022:1 | API:SAFE | OOR:BOR | context issue$\to$Index trait | No (n.a.) | No | 
 | Advisory-DB | RUSTSEC-2020-0022:2 | RAII (Imp) | DP:DUN | implement drop with uninit mem | No (n.a.)| No | 
...
\multirow{2}{*}{slice-deque } | 2018-20995 | pull/58 | GEN (Imp) | OOR:BO | error in boundary check + unsafe write | No (aldanor-deps) | No | 
| 2019-15543 | pull/66 | GEN (Imp) | OOR:BO | logical error$\to$memory misalignment | No (zimond) | No | 
\multirow{4}{*}{rust-smallvec } | 2019-15554 | issues/149 | IMP:GEN (Imp) | OOR:BO | logical error + unsafe write | No (ehuss) | No | 
 | Advisory-DB | issues/126 | RAII:UNWIND (Imp) | DP:DUN | init vector with mem:uninitialized() | No (mbrubeck) | No | 
 | 2019-15551 | issues/148 | GEN (Imp) | DP:UAF | logical error + unsafe deallocation | No (ehuss) | No | 
 | 2018-20991 | issues/96 | RAII:UNWIND (Imp) | DP:DF | buffer shrinking too late | No (Vurich) | No | 
...
\multirow{6}{*}{sized-chunks } | 2020-25791 | issues/11:unit | GEN (API) | OOR:BO | lack input consistency check + unsafe write | No (Qwaz-sec) | No | 
| 2020-25792 | issues/11:pair | GEN (API) | OOR:BO | lack input consistency check + unsafe write | No (Qwaz-sec) | No | 
| 2020-25793 | issues/11:From | GEN (API) | OOR:BO | lack input consistency check + unsafe write | No (Qwaz-sec) | No | 
| 2020-25796 | issues/11:InlineArray | GEN (API) | OOR:BO | no input check$\to$memory misalignment | No (Qwaz-sec) | No | 
| 2020-25794 | issues/11:clone | RAII:UNWIND (Imp) | DP:DUN | panic $\to$ drop uninitialized memory | No (Qwaz-sec)  | No | 
| 2020-25795 | issues/11:insert_from | RAII:UNWIND (Imp) | DP:DUN | panic $\to$ rop uninitialized memory | No (Qwaz-sec)  | No | 
\multirow{3}{*}{actix-net} | Advisory-DB | issues/91 |  | UAF | | sebzim4500 | No | 
| Advisory-DB | pull/158 | API:RC | UB$\to$UAF | replace Cell with RefCell | Shnatsel  | No | 
| Advisory-DB | issues/160 | API:RC | UB$\to$UAF | replace Cell with RefCell | Shnatsel | No | 
\multirow{3}{*}{actix-web} | Advisory-DB | issues/289 | API:MUTE | UB:DP | transmute immutable ref to mutable | seanmonstar | No | 
| | issues/289 | API:LIFE | DP | transmutate lifetime to static | seanmonstar | No | 
| | issues/289 | IMP:TRAIT+GENERIC | DP | Clone+Rc | seanmonstar | No | 
| Advisory-DB | issues/301 | API:CC+TRAIT+GENERIC | UAF | unsafe impl of Send for generics races Rc | seanmonstar-Mozilla | No | 
| Advisory-DB | issues/1321 | API:PAR | UAF |  | sebzim4500 | No | 
...
\multirow{4}{*}{futures-rs} | Advisory-DB | pull/2206 | API:GENERICS+LIFE | UAF | lack lifetime bound | Darksonn | No | 
| Advisory-DB | issues/2091 | IMP:LIFETIME+CC | UAF | return static ref to Send | goffrie | No | 
| Advisory-DB | issues/2239 | API:TRAIT+GENERICS+CC | RC$\to$UAF | lack send/sync bound | Qwaz-Sec | No | 
| Advisory-DB | issues/2050 | API:TRAIT+CC | UAF | impl sync for a structure with Cell<T> | okready | No | 
\multirow{2}{*}{pulse-binding-rust} | Advisory-DB | API:LIFETIME+RAII | UAF | lack lifetime bound: +PhantomData | jnqnfe | No | 
| Advisory-DB | issues/2050 | API:TRAIT+CC | UAF | impl sync for a structure with Cell<T> | okready | No | 
\multirow{5}{*}{parking_lot} | CVE-2020-35910 | issues/258:MappedMutexGuard | API:TRAIT+CC+GENERICS | RC$\to$UAF |  lack send bound | ammaraskar-Sec | No |  
| CVE-2020-35911 | issues/258:MappedRwLockReadGuard | API:TRAIT+CC+GENERICS | RC$\to$UAF |  lack sync bound | ammaraskar-Sec | No | 
| CVE-2020-35912 | issues/258:MappedRwLockWriteGuard | API:TRAIT+CC+GENERICS | RC$\to$UAF |  lack send bound | ammaraskar-Sec | No |  
| CVE-2020-35913 | issues/259:RwLockReadGuard | API:TRAIT+CC+GENERICS | RC$\to$UAF |  lack sync bound | ammaraskar-Sec | No | 
| CVE-2020-35914 | issues/259:RwLockWriteGuard | API:TRAIT+CC+GENERICS | RC$\to$UAF |  lack sync bound | ammaraskar-Sec | No | 
...
\multirow{2}{*}{image} | 2019-16138 | issues/980 | RAII:UNWIND* (Imp) | DP:DUN | unsafe allocation$\to$drop uninit mem/set_len() | No (64) | No | 
| CVE-2020-35916 | issues/1357 | IMP:MUT | UB | convert mutable ptr from const ptr | dodomorandi | No | 
...
\multirow{4}{*}{generator-rs } | 2019-16144 | issues/11 | IMP:RAII* (Imp) | DP:DF | shared mut aliases+auto drop | No (vOROn200) | No | 
  | Advisory-DB | issues/9 | RU:SAFE (API) | Unsound | func. sign.$\to$deref invalid/null pointer | No (jonas-schievink)| No | 
  | Advisory-DB | issues/13 | GEN (API) | Unsound | bad func. exposure  | No (jonas-schievink) | No | 
  | Advisory-DB  | issues/14 | GEN (API) | Unsound | bad func. exposure | No (jonas-schievink)  | No | 
...
\multirow{2}{*}{http } | 2020-25574 | issues/354 | RAII (Imp) | DP:DF | buf. shrinking too late (rely on drop) | No (Qwaz-sec) | No | 
    | Advisory-DB | issues/355* | RU:MUT (API) | Unsound | func. sign.$\to$multiple mutable refs/Send$\to$Sync  | No (Qwaz-sec)  | No | 
...
\multirow{2}{*}{flatbuffers } | Advisory-DB | issues/5825 | RU:SAFE (API) | Unsound | func. sign.: safety declaration | No (eduardosm)| No | 
 | Advisory-DB | issues/5530 | IMP* (Imp) | Unsound | logical error$\to$invalid bit pattern for bool | No (nagisa)| No | 
...
servo (exe)} |  \multirow{3}{*}{\makecell{GitHub\\317 unsafe}} | issues/1186 | RAII: (Imp) | DP:DF | impl Drop with unsafe | kmcallister | No | 
 | | issues/2412 | RAII: (Imp) | DP:DF  | *unsafe cast?  |  | No | 
 | | issues/14014 | RAII: (Imp) | CC$\to$UAF  |   | pcwalton-deps | No | 
 | | issues/14416 | RAII: (Imp) | DP:UAF  | FnBox  | pcwalton-deps | No | 
 | | issues/20158 | SAFETY: (API) | UB  | declare unsafe constructor as safe  | nox-org | No | 
 | | issues/21186 | Gen: (Imp) | CC:DR | Relaxed$\to$Acquire |  | No | 
 | | pull/26641 | Gen: (Imp) | UB | mem::transmute$\to$Box::into_raw() | dylni | No |
wasmer (exe) | GitHub | pull/1092 | RU:Safety (Imp) | OOR | lack input consistency check + unsafe constructor | MarkMcCaskey-dev | No | 
 | | pull/1206 | RU:Safety (API) | Unsound | possible to create shared mutable aliases | MarkMcCaskey-dev | No | 
 | | pull/1209 | RU:Safety (API) | CC:DR | multiple Cells for the same memory | MarkMcCaskey-dev | No | 
 | | pull/1837 | RU:Safety (API) | Unsound | declare unsafe API as safe | MarkMcCaskey-dev | No | 
 | | issues/1568 | RAII: | DP:UAF | shared mutable alises (slice+to_vec()) | Hywan-dev | No | 
alacritty (exe) | GitHub | pull/4397 | MemModel (Imp) | DP::UAF | shared mut aliases (ptr::copy$\to$clone) | kchibisov-dev | No | 
 | | pull/2176  | GEN (Imp) | UAF | logical error (+as_ref()) | aspurdy | No | 
diem-libra (exe) | GitHub | pull/1949 | RU:Lifetime (API) | DP:UAF | ineffective lifetime restriction | dtolnay-Rust  || wint}| \multirow{3}{*}{GitHub (72 unsafe)} | issues/599 | (Imp) | CC:RC | non-atomic cell for multi-thread apps | andrewhickman | No | 
 | | pull/611 | CC (Imp) | CC:RC | non-atomic cell for multi-thread apps | francesca64-dev | No | 
 | |  issues/1745 | GEN (Imp) | DP:UAF | logical error | qthree | No | 
| Tokio | CVE-2020-35922 | issues/1386 | FFI | UB | assumes the same layout of FFI | Nemo157-Rust | No | 
 | \multirow{3}{*}{GitHub (84 unsafe)} | *pull/254 |  | DP:UAF | | seanmonstar-dev | No | 
 | | issues/354 | Gen (Imp) | UB | uninit memory for Vec<u8> with set_len() | udoprog | No | 
 | | issues/593 |  | CC | | NPN |
...
| | issues/333 | RU: (API) | CC:DR | does not enforce FFI restrictions | DemiMarie/sagebind-deps | Yes | 
| | issues/340 | RU:SAFE (API) | DP:UAF | exposing unsafe cleanup APIs as safe | sagebind-deps | No | 
